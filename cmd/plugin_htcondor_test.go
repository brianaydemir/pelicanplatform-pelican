...
`)
	...
	// Wait for federation to be ready by polling the director health endpoint
	t.Log("Waiting for federation to be ready...")
	...
	// Create test input files in the origin
	testFilename1 := "test-input1.txt"
	testContent1 := "Hello from Pelican via HTCondor!\nThis is test data file 1.\n"
	testPath1 := filepath.Join(fed.Exports[0].StoragePrefix, testFilename1)
	require.NoError(t, os.WriteFile(testPath1, []byte(testContent1), 0644))

	testFilename2 := "test-input2.txt"
	testContent2 := "Second input file for HTCondor test!\nThis is test data file 2.\n"
	testPath2 := filepath.Join(fed.Exports[0].StoragePrefix, testFilename2)
	require.NoError(t, os.WriteFile(testPath2, []byte(testContent2), 0644))
...
ls -la
echo "Content of first input file:"
cat %s || echo "Failed to read first input file"
echo "Content of second input file:"
cat %s || echo "Failed to read second input file"
echo "Creating output file..."
echo "Output from HTCondor job" > test-output.txt
echo "Listing directory again:"
ls -la
echo "Job completed"
`, testFilename1, testFilename2)
...
# Transfer the input files from the federation
transfer_input_files = %s/test/%s,%s/test/%s
...
`, scriptPath, jobDir, jobDir, jobDir, federationURL, testFilename1, federationURL, testFilename2, outputFile, tlsCAPath)
	...
	if outContent, err := os.ReadFile(outFile); err == nil {
		...
		assert.Contains(t, string(outContent), testContent1, "Job should have read first input file")
		assert.Contains(t, string(outContent), testContent2, "Job should have read second input file")
	}
}
...
func stopCondorMaster(cmd *exec.Cmd, t *testing.T) {
	if cmd != nil && cmd.Process != nil {
		...
		// Poll to see if the process still exists
