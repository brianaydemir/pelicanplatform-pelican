...
// Package configtest provides shared TLS test helpers for packages that
...
// This matches the format written by config.GenerateCACert / GeneratePrivateKey.
...
func NewTLSServerForTest(t testing.TB, handler http.Handler) *httptest.Server {
	...
	require.NoError(t, err, "Failed to create test TLS listener")
